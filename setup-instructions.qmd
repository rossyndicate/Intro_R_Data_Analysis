...
1.  **First** **Install R** from CRAN: <https://cran.r-project.org/>. Choose the correct download link based on your operating system (Mac, Windows, Linux)
    - Windows: "Download R for Windows" → "base" → download the installer
    - Mac: pick the **arm64** `.pkg` for Apple Silicon (M1/M2/M3/M4), **x86_64** for older Intel Macs.
2.  **Second, Install RStudio IDE at** <https://docs.posit.co/ide/user/#rstudio-ide-oss-downloads> for your operating system. Accept all defaults.
    - If you have an older MacOS version (pre-14), you can download other supported versions here: <https://docs.posit.co/supported-versions/rstudio.html>

*Note: If you already have R installed, you **must have at least version 4.5.0. or greater**, but it is best to have the most recent version installed (4.6.1)*
...
**Install the [{tidyverse}](https://tidyverse.org/) package.** The Tidyverse is actually a collection of multiple R packages designed for data manipulation, exploration, and visualization that you are likely to use in every day data analysis. When you install the Tidyverse, it installs all of these packages, and you can later load all of them in your R session with `library(tidyverse).` Since you are installing multiple packages here this may take a little while.

```{r eval = FALSE}
install.packages("tidyverse")
```

**Install the [{palmerpenguins}](https://allisonhorst.github.io/palmerpenguins/) package.** This is a data package that installs a couple of spreadsheets you can load and work with in R.
