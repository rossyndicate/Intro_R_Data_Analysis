...
You can set up your session by executing your set up script we created in [Lesson 1](introduction-to-r-rstudio-and-r-markdown.qmd#write-a-set-up-script)
...
You may also come across a second pipe, `|>`, called the **native** or **base R
pipe**. It was added to R itself in version 4.1 (2021), so it works without
loading any packages. For everything we do in this course, `x |> f()` and
`x %>% f()` mean exactly the same thing: take the object on the left and pass
it as the first argument to the function on the right. Read both as "then."

```{r eval = FALSE}
# these two are equivalent
penguins %>% filter(species == "Adelie") %>% select(bill_length_mm)

penguins |> filter(species == "Adelie") |> select(bill_length_mm)
```

Use whichever you prefer, but stay consistent within a script. We use `%>%` in
these materials because it is what you will see in most existing tidyverse code
and documentation, but you are welcome to use `|>` in your own work. The main
difference to be aware of is that `%>%` lets you use a `.` as a placeholder to
send the piped object somewhere other than the first argument (e.g.
`df %>% lm(y ~ x, data = .)`), which the native pipe handles differently. If
you are not using that placeholder, the two are interchangeable.
