...
Let's re-explore the difference in weight of cutthroat trout in clear cut (CC) and old growth (OG) forest types from our [T-test and ANOVA lesson](data-analysis-t-test-and-anova.qmd). We want to see how sample size affects our ability to detect this difference. Therefore, our research question is: "Is there a significant difference in weight between old growth and clear cut forest types?" **We will set our significance level at 0.05 (i.e., our test's p-value must be below 0.05 for us to reject the null hypothesis).**
...
HINTS: the number of observations you want to subset to will be an argument of the function. The code above can be used as the basis of the function. You can refer back to the [Write Functions](https://github.com/rossyndicate/primers?tab=readme-ov-file) primer lessons for guidance, specifically [How to write a function](https://geocentroid.shinyapps.io/how-to-write-a-function/).
